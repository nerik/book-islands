...
const getClustersSupercluster = (features, bbox) => {
  ...
  const initialClusters = initialClustersWithNoise
    ...
    .map(cluster => {
      ...
      return {
        ...cluster,
        initialLeaves: initialIndex.getLeaves(cluster.properties.cluster_id, Infinity).map(l => l.properties.id),
        properties: {
  ...
  const metaClusters = metaClustersWithNoise.map(metaCluster => {
    ...
    if (metaClusterId === undefined) {
      ...
      return {
        ...
        leaves: initialClusters.find(c => c.properties.initialClusterId === initialClusterId).initialLeaves
      }
    }
    const allInitialLeaves = metaIndex.getLeaves(metaClusterId, Infinity)
      .map(metaLeaf => {
...
const getClustersDbscan = (features) => {
  ...
  const clusteredFeatures = turf.clustersDbscan(turf.featureCollection(
