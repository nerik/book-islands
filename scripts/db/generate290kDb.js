...
WITH books_with_cover AS (
  ...
  FROM `cilex-books-245511.cilex_books.290k_mid_resume` books
  INNER JOIN `cilex-books-245511.cilex_books.books_cover` books_cover ON books.id = books_cover.id
 )
SELECT
  books_with_cover.id as id, books_with_cover.category, books_with_cover.relatedness_signal_projected_query_popularity as score, books_with_cover.title, books_with_cover.cover_url, books_info.author, books_info.summary, books_info.description , books_info.publication_year, books_info.publication_year_extra
FROM books_with_cover INNER JOIN `cilex-books-245511.cilex_books.290k_mid_resume` books_info ON books_with_cover.id = books_info.id
*/
...
const { BOOKS_290K_CSV, BOOKS_DB_290K, BOOKS_DB_TABLE } = require('../constants')

const file = fs.createReadStream(BOOKS_290K_CSV)
const TABLE_FIELDS = [
  { name: 'id', type: 'STRING' },
...
if (fs.existsSync(BOOKS_DB_290K)) {
  console.log('Deleting previous database')
  fs.unlinkSync(BOOKS_DB_290K)
}

let db = new sqlite3.Database(BOOKS_DB_290K, (err) => {
  if (err) {
...
const insertData = () => {
  ...
  Papa.parse(file, {
    ...
    complete: function() {
      ...
      console.log(`Closing ${BOOKS_DB_290K}`)
      db.close()
    },
  })
}
